...
int main(int argc, char *argv[]){
	...
	int random = ((unsigned)rand() << 17) | ((unsigned)rand() << 2) | ((unsigned)rand() & 3);

	// Transfer data from host to device
	hipEvent_t start, stop;
	initializeEvents(&start, &stop);
	HIP_SAFE_CALL(hipMemset(deviceIn, random, size*sizeof(int)));
	double MemSetTime = finalizeEvents(start, stop);
	...
	HIP_SAFE_CALL(hipMemSet(hostOut, deviceOut, size*sizeof(int), hipMemSetDeviceToHost));
